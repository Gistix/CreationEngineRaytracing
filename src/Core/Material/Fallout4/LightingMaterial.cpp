...
void LightingMaterial::UpdateData(RE::BSShaderMaterial* shaderMaterial)
{
	...
	lightingData->Smoothness = lightingShaderMaterial->smoothness;
...
void LightingMaterial::UpdateTextures(RE::BSShaderMaterial* shaderMaterial)
{
	...
	if (m_SmoothnessSpecMaskTexture.Update(lightingShaderMaterial->smoothnessSpecMaskTexture, renderer->GetBlackTextureDescriptor()))
		lightingData->SmoothnessSpecMaskTexture = m_SmoothnessSpecMaskTexture.texture.GetDescriptorIndex();
}