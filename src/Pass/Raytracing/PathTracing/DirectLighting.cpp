...
{
	DirectLighting::DirectLighting(Renderer* renderer, SceneTLAS* sceneTLAS)
	...
	void DirectLighting::ResolutionChanged([[maybe_unused]] uint2 resolution)
	{
		m_BindingSetDirty.fill(true);
	}

	void DirectLighting::SettingsChanged(const Settings& settings)
	{
	...
	void DirectLighting::CreateBindingLayout()
	{
		...
		globalBindingLayoutDesc.bindings = {
			...
			nvrhi::BindingLayoutItem::Texture_SRV(13),         // Depth
			nvrhi::BindingLayoutItem::Texture_SRV(14),         // Albedo
			nvrhi::BindingLayoutItem::Texture_SRV(15),         // EmissiveMetallic
			nvrhi::BindingLayoutItem::Texture_SRV(16),         // NormalRoughness
			nvrhi::BindingLayoutItem::Texture_SRV(17),         // Material
	...
	bool DirectLighting::CreateRayTracingPipeline()
	{
		...
		auto rayGenLib = ShaderUtils::CompileShaderLibrary(device, L"data/shaders/raytracing/PathTracing/DirectLighting/RayGeneration.hlsl", defines);
		if (!rayGenLib)
			return false;
	...
	bool DirectLighting::CreateComputePipeline()
	{
		auto defines = Util::Shader::GetDXCDefines(m_Defines);
		...
		auto rayGenBlob = ShaderCache::GetShader(L"data/shaders/raytracing/PathTracing/DirectLighting/RayGeneration.hlsl", defines, ShaderStage::Compute);
		if (!rayGenBlob)
			return false;
	...
	void DirectLighting::CreatePipeline()
	{
	...
	void DirectLighting::CheckBindings()
	{
		...
		bindingSetDesc.bindings = {
			...
			nvrhi::BindingSetItem::Texture_SRV(13, textureManager.GetTexture(RenderTarget::ClipDepth)),
			nvrhi::BindingSetItem::Texture_SRV(14, textureManager.GetTexture(RenderTarget::Albedo)),
			nvrhi::BindingSetItem::Texture_SRV(15, textureManager.GetTexture(RenderTarget::EmissiveMetallic)),
			nvrhi::BindingSetItem::Texture_SRV(16, textureManager.GetTexture(RenderTarget::NormalRoughness)),
			nvrhi::BindingSetItem::Texture_SRV(17, textureManager.GetTexture(RenderTarget::Material)),
	...
	void DirectLighting::Execute(nvrhi::ICommandList* commandList)
	{
		CheckBindings();
