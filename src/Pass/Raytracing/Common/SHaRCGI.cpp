...
namespace Pass::Raytracing::Common
{
	...
	void SHaRCGI::CreateUpdateComputePipeline()
	{
		auto device = GetRenderer()->GetDevice();
		...
		if (!rayGenBlob) {
			logger::error("SHaRCGI::CreateUpdateComputePipeline - Failed to compile update shader.");
			return;
		}

		m_UpdatePass.m_ComputeShader = device->createShader({ nvrhi::ShaderType::Compute, "SHaRC GI Update Shader", "Main" }, rayGenBlob->GetBufferPointer(), rayGenBlob->GetBufferSize());
		if (!m_UpdatePass.m_ComputeShader) {
			logger::error("SHaRCGI::CreateUpdateComputePipeline - Failed to create update shader.");
			return;
		}
		...
		m_UpdatePass.m_ComputePipeline = device->createComputePipeline(pipelineDesc);
		if (!m_UpdatePass.m_ComputePipeline) {
			logger::error("SHaRCGI::CreateUpdateComputePipeline - Failed to create update pipeline.");
			return;
		}
	}

	void SHaRCGI::CreateUpdateRayTracingPipeline()
	{
		auto defines = Util::Shader::GetDXCDefines(m_Defines);
		...
		if (!rayGenLib) {
			logger::error("SHaRCGI::CreateUpdateRayTracingPipeline - Failed to compile ray gen library.");
			return;
		}
		...
		if (!m_UpdatePass.m_RayPipeline) {
			logger::error("SHaRCGI::CreateUpdateRayTracingPipeline - Failed to create ray tracing pipeline.");
			return;
		}

		auto shaderTableDesc = nvrhi::rt::ShaderTableDesc()
			.enableCaching(5)
			.setDebugName("SHaRC GI Update Shader Table");

		m_UpdatePass.m_ShaderTable = m_UpdatePass.m_RayPipeline->createShaderTable(shaderTableDesc);
		if (!m_UpdatePass.m_ShaderTable) {
			logger::error("SHaRCGI::CreateUpdateRayTracingPipeline - Failed to create shader table.");
			return;
		}
	...
	void SHaRCGI::CheckBindings()
	{
		...
		if (!m_UpdatePass.m_BindingSets[currentSlot]) {
			for (const auto& binding : bindingSetDesc.bindings) {
				logger::info("SHaRCGI::CheckBindings - {}, {}, 0x{:08X}", magic_enum::enum_name(binding.type), binding.slot, reinterpret_cast<uintptr_t>(binding.resourceHandle));
			}
		}
