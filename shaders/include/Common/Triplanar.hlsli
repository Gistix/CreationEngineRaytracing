...
namespace Triplanar
{
	...
	float3 GetWeights(float3 vertexNormal, float3 faceNormal, float sharpness = BLEND_SHARPNESS)
	{
        ...
        return w / (dot(w, 1.0) + DIV_EPSILON);
    }

	/// Weighted triplanar sample blending all 3 planes — stable for alpha/fade values.
	float4 Sample(Texture2D<float4> tex, SamplerState samp, float lod, float3 worldPos, float3 weights, float scale)
	{
        return tex.SampleLevel(samp, worldPos.yz * scale, lod) * weights.x +
		       tex.SampleLevel(samp, worldPos.xz * scale, lod) * weights.y +
		       tex.SampleLevel(samp, worldPos.xy * scale, lod) * weights.z;
    }
