...
void DefaultMaterial(inout Surface surface, in float2 texCoord0, in float4 vertexColor, in float3 normalWS, in float3 tangentWS, in float3 bitangentWS, in Mesh mesh, float4 boneRotation)
{
    ...
    if (material.Type == Type::TruePBR)
    {
        ...
        surface.Emissive = GamutTransform(emissive) * EmitColorToLinear(mesh.Properties.EmissiveColor.rgb) * mesh.Properties.EmissiveColor.a * EmitColorMult() * (surface.Primary ? 1.0f : LIGHTINGSETTINGS.Emissive);
        ...
        if (pbr.PBRFlags & PBR::Flags::Subsurface)
        {
            ...
            {
                surface.SubsurfaceData.ScatteringColor = GamutTransform(subsurfaceColor.rgb * pbr.FeatureColor.rgb);
