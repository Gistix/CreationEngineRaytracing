...
namespace LandMaterialInternal
{
    float3 VanillaColorLinear(float3 color)
    {
        return saturate(LLON ? pow(abs(color), LLSETTINGS.colorGamma) * LLSETTINGS.vanillaDiffuseColorMult : ColorToLinear(color));
    }

    float4 VanillaColorLinear(float4 color)
    {
        return float4(VanillaColorLinear(color.rgb), color.a);
    }
}
...
void LandMaterial(inout Surface surface, in float2 texCoord0, in float4 vertexColor, float3 normalWS, float3 tangentWS, float3 bitangentWS, float4 landBlend0, float4 landBlend1, in Mesh mesh)
{
    ...
    if (material.Type == Type::TruePBR)
    {
        blendedLand += (pbrFlags & PBR::TerrainFlags::LandTile0PBR) ? PBRColorScale(land1) : LandMaterialInternal::VanillaColorLinear(land1);
        blendedLand += (pbrFlags & PBR::TerrainFlags::LandTile1PBR) ? PBRColorScale(land2) : LandMaterialInternal::VanillaColorLinear(land2);
        blendedLand += (pbrFlags & PBR::TerrainFlags::LandTile2PBR) ? PBRColorScale(land3) : LandMaterialInternal::VanillaColorLinear(land3);
        blendedLand += (pbrFlags & PBR::TerrainFlags::LandTile3PBR) ? PBRColorScale(land4) : LandMaterialInternal::VanillaColorLinear(land4);
        blendedLand += (pbrFlags & PBR::TerrainFlags::LandTile4PBR) ? PBRColorScale(land5) : LandMaterialInternal::VanillaColorLinear(land5);
        blendedLand += (pbrFlags & PBR::TerrainFlags::LandTile5PBR) ? PBRColorScale(land6) : LandMaterialInternal::VanillaColorLinear(land6);
    ...
    {
        blendedLand = LandMaterialInternal::VanillaColorLinear(land1 + land2 + land3 + land4 + land5 + land6);
        blendedLand.rgb *= LandMaterialInternal::VanillaColorLinear(vertexColor.rgb);
    ...
    surface.Albedo = GamutTransform(blendedLand.rgb);
