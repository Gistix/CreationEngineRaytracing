...
#if defined(USE_SER) || defined(__INTELLISENSE__)
...
#if USE_NVAPI_HIT_OBJECT_EXTENSION || defined(__INTELLISENSE__)
...
#define SER_HIT_OBJECT_INIT_FROM_RAYQ(_tlas, _outHit, _rayQuery ) \
        ...
        NvMakeHitWithRecordIndex( __shaderTableIndex, _tlas, _rayQuery.CommittedInstanceIndex(), _rayQuery.CommittedGeometryIndex(), _rayQuery.CommittedPrimitiveIndex(), 0, __rayDesc, __attrib, _outHit );
#define SER_REORDER_HIT( _hit, _sortBits, _numberOfBits ) \
        NvReorderThread( _hit, _sortBits, _numberOfBits );
#define SER_INVOKE_HIT(_tlas, _hit, _payload ) \
        NvInvokeHitObject(_tlas, _hit, _payload);
